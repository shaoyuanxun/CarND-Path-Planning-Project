...
void Vehicle::_realise_state(States astate, vector<vector<double>> sensor_fusion){
  ...
  // compute collision on the start and end lane
  for (int i = 0; i < sensor_fusion.size(); i++) {
    float car_d = sensor_fusion[i][6]; 
    if ((car_d < (2 + 4 * trajectory.lane_start + 2)) && car_d > ( 2 + 4 * \
          trajectory.lane_start - 2) ) {
    ...
    if ((car_d < (2 + 4 * trajectory.lane_end + 2)) && car_d > ( 2 + 4 * \
          trajectory.lane_end - 2) ) {
      double vx = sensor_fusion[i][3];
      double vy = sensor_fusion[i][4];
      double check_speed = sqrt(vx * vx + vy * vy);
      ...
      if ((trajectory.lane_end != trajectory.lane_start &&
            (abs(dist_to_collision) < 30)) || ( trajectory.lane_end == \
              trajectory.lane_start && check_car_s >= s && \
      ...
      if (!collider.collision && collider.closest_approach > \
          abs(dist_to_collision) ) {
          collider.closest_approach = abs(dist_to_collision);
          collider.target_speed = check_speed * MS_TO_MPH;
      }
    }
